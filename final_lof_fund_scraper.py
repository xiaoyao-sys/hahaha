...
def generate_and_save_fund_summary():
    ...
    try:
        ...
        # 只对高溢价基金查询限额信息
        if FUND_LIMIT_AVAILABLE and get_fund_limit is not None and high_premium_funds:
            print(f"正在查询 {len(high_premium_funds)} 只高溢价基金的限额信息...")
            for fund in high_premium_funds:
                try:
                    fund['limit_info'] = get_fund_limit(fund['code']) or ''
                except Exception as e:
                    print(f"获取基金 {fund['code']} 限额信息时出错: {e}")
                    fund['limit_info'] = '获取失败'
